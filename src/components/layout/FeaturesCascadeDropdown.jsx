...
export function FeaturesCascadeDropdown({ isOpen, onClose, onNavigate }) {
  const [activeCategory, setActiveCategory] = useState(featureCategoriesList[0])

  if (!isOpen) return null

  const activeSubFeatures = getFeaturesByCategory(activeCategory.id)

  const handleCategoryClick = (cat) => {
    setActiveCategory(cat)
  }

  const handleSubItemClick = (e, catId, featureSlug) => {
    e.preventDefault()
    if (onNavigate) {
        ...
        <div>
          ...
          <div className="space-y-1 max-h-[380px] overflow-y-auto pr-1">
            {activeSubFeatures.map((sub, idx) => (
              <a
                key={idx}
                href={`#feature/${activeCategory.id}/${sub.slug}`}
                onClick={(e) => handleSubItemClick(e, activeCategory.id, sub.slug)}
